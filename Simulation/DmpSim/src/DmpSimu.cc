...
int main(int argc, char** argv)
{
  ...
  DmpG4RunManager* runManager = new DmpG4RunManager();
    ...
    // set an ROOT data manager
  DmpRootEventDataManager*  rootEventDataManager = new DmpRootEventDataManager();
  ...
  dmpSimuPrimaryGeneratorAction->SetDmpRootEventDataManager(rootEventDataManager);

  // Set optional user action classes
  DmpSimuRunAction* runAction = new DmpSimuRunAction(rootEventDataManager,dmpSimuPrimaryGeneratorAction);
  runManager->SetUserAction(runAction);
  
  DmpSimuEventAction* eventAction = new DmpSimuEventAction(rootEventDataManager);
  runManager->SetUserAction(eventAction);
  
  DmpSimuTrackingAction* tracking_action = new DmpSimuTrackingAction();
  runManager->SetUserAction(tracking_action);
  
  DmpSimuSteppingAction* stepping_action = new DmpSimuSteppingAction(rootEventDataManager);
