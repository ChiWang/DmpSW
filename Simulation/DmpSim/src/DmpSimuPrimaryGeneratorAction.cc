...
DmpSimuPrimaryGeneratorAction::DmpSimuPrimaryGeneratorAction()
  :rndmFlag("off"),nSourceType(0),nSpectrumType(0),genOnly(false),useGenFilter(false),passGenFilter(true),
...
void DmpSimuPrimaryGeneratorAction::SetupSourceType(G4int val)
{
  ...
  G4double r0 = std::max(0.5*PayLoadSizeXY,0.5*PayLoadSizeZ)+50.;
