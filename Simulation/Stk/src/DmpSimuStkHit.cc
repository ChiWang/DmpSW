  ...
  :G4VHit()
{
  ...
  pos = right.pos;
  ChrgTrkEnergy = right.ChrgTrkEnergy;
}
...
int DmpSimuStkHit::operator==(const DmpSimuStkHit& right) const
{
  return((EdepSil==right.EdepSil)&&(NStrip==right.NStrip)&&(IsXPlane==right.IsXPlane)&& (pos==right.pos) &&
	 (EdepSilPrim==right.EdepSilPrim)&&(trackID==right.trackID)&&(PrimTrackID==right.PrimTrackID)&&
	 (nTracks==right.nTracks)&&(nTracksBack==right.nTracksBack)&&(ChrgTrkEnergy==right.ChrgTrkEnergy));
}
