...
{ 
...
#ifdef STK_NTUPLER_DEBUG_MODE
  G4cout<<G4endl;
  G4cout<<"[DmpStkSensitiveDetector::ProcessHits] tile->GetName()" <<tile->GetName()<<G4endl;
  G4cout<<"[DmpStkSensitiveDetector::ProcessHits]   TileNumber: "<<TileNumber<<G4endl;
  //G4cout<<"[DmpStkSensitiveDetector::ProcessHits]   TileName: "<<TileName<<G4endl;
  G4cout<<"[DmpStkSensitiveDetector::ProcessHits]   PlaneNumber: "<<PlaneNumber<<G4endl;
  //G4cout<<"[DmpStkSensitiveDetector::ProcessHits]   PlaneName: "<<PlaneName<<G4endl;
  G4cout<<"[DmpStkSensitiveDetector::ProcessHits]   LayerNumber: "<<LayerNumber<<G4endl;
  G4cout<<"[DmpStkSensitiveDetector::ProcessHits]   BlockNumber: "<<BlockNumber<<G4endl;
  G4cout<<"[DmpStkSensitiveDetector::ProcessHits] strip->GetName(): "<<strip->GetName()<<G4endl;
  //G4cout<<"[DmpStkSensitiveDetector::ProcessHits]   StripName: "<<StripName<<G4endl;
  G4cout<<"[DmpStkSensitiveDetector::ProcessHits]   StripNumber: "<<StripNumber<<G4endl;
#endif
  ...
  ***/
...
#ifdef STK_NTUPLER_DEBUG_MODE
  G4cout<<"[DmpStkSensitiveDetector::ProcessHits] The hit is on an X silicon plane"<<G4endl;
#endif
	...
	(*TrackerCollection)[ThitXID[NChannel]]->AddSilPrim(edep);
      ...
      if(aStep->GetTrack()->GetDefinition()->GetPDGCharge()){
        (*TrackerCollection)[ThitXID[NChannel]]->AddChrgTrkEnergy(aStep->GetTrack()->GetTotalEnergy());
      }
  ...
  if(dmpDetector->GetStkDetectorDescription()->isVolumeSTKActiveTileY(tile->GetName())){

#ifdef STK_NTUPLER_DEBUG_MODE
  G4cout<<"[DmpStkSensitiveDetector::ProcessHits] The hit is on an Y silicon plane"<<G4endl;
#endif
	...
	//G4cout << "Y new strip: track ID " << aStep->GetTrack()->GetTrackID() <<  " edp " << edep<< " prim edp " << TrackerHit->GetEdepSilPrim() << " NChannel " << NChannel<< " isPrim " << isPrim <<G4endl;
      ...
      if(aStep->GetTrack()->GetDefinition()->GetPDGCharge()){
        (*TrackerCollection)[ThitYID[NChannel]]->AddChrgTrkEnergy(aStep->GetTrack()->GetTotalEnergy());
      }
